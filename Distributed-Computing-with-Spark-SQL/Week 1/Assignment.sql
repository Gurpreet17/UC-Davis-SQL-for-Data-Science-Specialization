...
WHERE total = (
    SELECT MAX(total)
    FROM (
        SELECT COUNT(`Ignition Cause`) AS total 
        FROM fireIncidents
        GROUP BY `Ignition Cause`
    ) AS subquery
);
...
-- TODO
SELECT *
FROM fireCalls c INNER JOIN fireIncidents i
ON c.Battalion = i.Battalion
...
-- TODO
SELECT COUNT('Incident Number') AS total
FROM fireCalls c INNER JOIN fireIncidents i
ON c.Battalion = i.Battalion
